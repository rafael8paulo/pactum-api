# Spec: Editar e Remover Despesa

## Purpose

Define o contrato para a edição completa de uma despesa via `PUT /api/v1/despesas/{id}` e para a remoção permanente via `DELETE /api/v1/despesas/{id}`.
