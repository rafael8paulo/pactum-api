# Spec: Editar e Remover Receita

## Purpose

Define o comportamento esperado dos endpoints `PUT /api/v1/receitas/{id}` e `DELETE /api/v1/receitas/{id}` para edição completa e remoção de receitas, incluindo o tratamento da exceção `ReceitaNaoEncontradaException`.
